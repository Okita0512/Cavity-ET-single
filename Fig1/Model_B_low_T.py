...
w = - np.linspace(0, 3.0 / conv, 2000)
Prefactor = np.sqrt(np.pi * beta / lam)
L_max = 20           # number of Frank-Condon terms
Exp = np.zeros((L_max, 2000), dtype= float)
...
plt.semilogy(t * conv, y_c * conv, 'o-', color = '#0385FF', markersize = 4, markeredgewidth = 2, markeredgecolor = "#0385FF", markerfacecolor = '#0385FF', label = 'Inside Cavity (FGR)')
...
x1, x2 = 0, 1.0      # x-axis range: (x1, x2)
y1, y2 = 1e-9, 1e-1     # y-axis range: (y1, y2)
...
plt.savefig("Model B - low T.pdf", bbox_inches='tight')