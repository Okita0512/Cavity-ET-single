...
def bathParam_cav(Lam, Gamma, ws, ndof):
    ...
    ω =  np.linspace(1e-8 * meV_to_au, w_c + 50 * Gamma, ndof)
    dω = ω[1] - ω[0]
    for j in range(ndof):
...
plt.semilogy(- w * conv, k_Marcus_inn * conv, '-', linewidth = 3, color = "orange", label = r'Inside Cavity (GMJ, $\mathcal{Q}$ = 5)')
...
plt.savefig("Model_B_Q = 5_GMJ.pdf", bbox_inches='tight')