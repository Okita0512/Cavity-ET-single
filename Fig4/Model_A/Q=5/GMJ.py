...
def bathParam_cav(Lam, Gamma, ws, ndof):
    ...
    ω =  np.linspace(1e-8 * meV_to_au, w_c + 200 * Gamma, ndof)
    dω = ω[1] - ω[0]
    for j in range(ndof):
...
plt.semilogy(- w * conv, k_Marcus_in2 * conv, '-', linewidth = 3, color = "orange", label = r'Inside Cavity (GMJ, $\mathcal{Q}$ = 5)')

data1 = np.loadtxt("dG", dtype = float)
data2 = np.loadtxt("k_in", dtype = float)
plt.semilogy(- data1, data2, 'o', markersize = 6, markeredgewidth = 2, markeredgecolor = "#0385FF", markerfacecolor = '#0385FF', label = r'Inside Cavity (FGR, $\mathcal{Q}$ = 5)')
...
plt.savefig("Model_A_Q = 5_GMJ.pdf", bbox_inches='tight')