...
plt.semilogy(- w * conv, k_Marcus_in * conv, '--', linewidth = 3, color = "#FD0000", label = 'Inside Cavity (MJ, no loss)')
...
def bathParam_cav(Lam, Gamma, ws, ndof):
    ...
    ω =  np.linspace(1e-8 * meV_to_au, w_c + 0.5 * Gamma, ndof)
    dω = ω[1] - ω[0]
    for j in range(ndof):
...
# instanteneous Marcus
Gamma_c = 10000 * meV_to_au
ndof_c = 1000
...
for i in range(len(C)):
    Expp_2 += C[i]**2 * np.exp(- beta * (- w - lam - W[i])**2 / (4 * lam))

k_Marcus_in2 = Prefactor * (Delta**2 * Expp_1 + t_DA**2 * Expp_2)

plt.semilogy(- w * conv, k_Marcus_in2 * conv, '-', linewidth = 3, color = "orange", label = r'Inside Cavity (GMJ, $\mathcal{Q}$ = 0.2)')

data1 = np.loadtxt("dG", dtype = float)
data2 = np.loadtxt("k_in", dtype = float)
plt.semilogy(- data1, data2, 'o', markersize = 6, markeredgewidth = 2, markeredgecolor = "#0385FF", markerfacecolor = '#0385FF', label = r'Inside Cavity (FGR, $\mathcal{Q}$ = 0.2)')
...
plt.savefig("Model_A_Q = 0.2_GMJ.pdf", bbox_inches='tight')