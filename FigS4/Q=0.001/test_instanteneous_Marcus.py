...
def bathParam_cav(Lam, Gamma, ws, ndof):
    ...
    for d in range(ndof):
        ...
        c[d] =  np.sqrt(2 * Lam / (ndof + 1)) * ω[d]
    
    return c, ω

# instanteneous Marcus
Gamma_c = 2e6 * meV_to_au
ndof_c = 100000
...
plt.semilogy(- w * conv, k_Marcus_in2 * conv, '--', linewidth = 3, color = "orange", label = 'Marcus Inside Cavity_Q = 0.001')
...
plt.savefig("Q = 0.001_test.pdf", bbox_inches='tight')