...
font = {'family':'Times New Roman', 'weight': 'roman', 'size':15}
...
L_max = 6           # number of Frank-Condon terms
...
for m in range(L_max):
    ...
    k_Marcus_inn += Delta**2 * FC(0, m)**2 * Exp[m, :] 
    if m >= 1:
        k_Marcus_in += t_DA**2 * FC(1, m)**2 * Exp[m, :] 
k_Marcus_in = k_Marcus_in * Prefactor
k_Marcus_inn = k_Marcus_inn * Prefactor

# ==============================================================================
plt.semilogy(- w * conv, k_Marcus_out * conv, '-', linewidth = 3, color = '#444444', label = 'Outside Cavity')
plt.semilogy(- w * conv, k_Marcus_in * conv, '-', linewidth = 3, color = "#FD0000", label = 'Eq. 39 of Ref. 1')
plt.semilogy(- w * conv, k_Marcus_inn * conv, '--', linewidth = 3, color = "#1909F8", label = 'Eq. 23 of this work')
