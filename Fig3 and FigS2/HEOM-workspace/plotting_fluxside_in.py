...
data1 = np.loadtxt("./rho-in.dat", dtype = float)
...
ratio = 1e-3
plt.plot(time_1[0:-1] / (1000 * fs_to_au), rate_1 / ratio, "-", linewidth = lw, color = 'tab:red', label = r"Inside Cavity")

# x and y range of plotting 
time = 5
y1, y2 = -1.5, 1.8     # y-axis range: (y1, y2)
...
ax.set_ylabel(r'$k\ (\times 10^{-3}\ \mathrm{eV} / \hbar)$', size = 24)
ax.legend(frameon = False, loc = 'upper left', prop = font_legend, markerscale = 1)



# plt.show()
plt.savefig("fig_flux_side_in.pdf", bbox_inches='tight')
