...
plt.plot(data1[:, 0] / (1000 * fs_to_au), PRt_1, "-", linewidth = lw, color = 'tab:blue', label = r"Outside Cavity (HEOM)")

x_data1 = data1[:, 0]
dt = x_data1[1] - x_data1[0]
y_data1 = PRt_1
t_plateau = 2000 * fs_to_au
cutoff = int(t_plateau / dt)
params, params_covariance = opt.curve_fit(exp_func, x_data1[cutoff:], y_data1[cutoff:], p0=[-1.0, -2e-6, 1.0])
print("=== Outside Cavity ===")
print("Marcus rate: 4.93e-5")
print("Estimated rate:", - params[1] * conv)

plt.plot(x_data1[cutoff:] / (1000 * fs_to_au), exp_func(x_data1, *params)[cutoff:], '--', linewidth = lw, color = 'k', label = 'Outside Cavity (Fitted)')
...
plt.plot(data2[:, 0] / (1000 * fs_to_au), PRt_2, "-", linewidth = lw, color = 'tab:red', label = r"Inside Cavity (HEOM)")

x_data2 = data2[:, 0]
dt = x_data2[1] - x_data2[0]
y_data2 = PRt_2
t_plateau = 2000 * fs_to_au
cutoff = int(t_plateau / dt)
params, params_covariance = opt.curve_fit(exp_func, x_data2[cutoff:], y_data2[cutoff:], p0=[-1.0, -5e-6, 1.0])
print("=== Inside Cavity ===")
print("Marcus-Jortner rate: 1.06e-4")
print("Estimated rate:", - params[1] * conv)

plt.plot(x_data2[cutoff:] / (1000 * fs_to_au), exp_func(x_data2, *params)[cutoff:], '--', linewidth = lw, color = 'navy', label = 'Outside Cavity (Fitted)')
