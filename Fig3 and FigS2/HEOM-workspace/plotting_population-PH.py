    ...
    for j in range(n_el):
        for m in range(n_ph):
            for n in range(n_ph):
                ...
                PRt_2 += data2[:, 2 * (nfock * (n_el * m + i) + (n_el * n + j)) + 1] * delta(i, j) * n * delta(m, n)

plt.plot(data2[:, 0] / (1000 * fs_to_au), PRt_2, "-", linewidth = 1.0, color = 'tab:orange', label = r"Inside Cavity")

# x and y range of plotting 
time = 5
y1, y2 = 0.0, 0.6     # y-axis range: (y1, y2)
...
ax.set_ylabel(r'$N(t)$', size = 24)
# ax.legend(frameon = False, loc = 'lower right', prop = font_legend, markerscale = 1)



# plt.show()
plt.savefig("population-PH.pdf", bbox_inches='tight')
